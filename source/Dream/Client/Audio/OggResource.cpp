...
// MARK: OggReader
			...
			OggReader::OggReader (Shared<std::istream> input_stream) : _input_stream(input_stream)
			{
...
// MARK: OggStream
			...
			OggStream::OggStream (Ptr<Source> source, ALenum format, ALsizei frequency, const Ptr<IData> data) : Stream(source, format, frequency), _reader(data->input_stream()), _data(data), _loop(true)
			{
			}
...
// MARK: OggResource
			...
			OggResource::OggResource (const Ptr<IData> data) : _data(data)
			{
