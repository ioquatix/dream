	...
	namespace Client
	{
		namespace Graphics
		{
			...
			public:
				...
				void render(const std::vector<Vec3> &) const;
				
				void render(const Geometry::LineSegment<2> &) const;
				void render(const Geometry::LineSegment<3> &) const;
								
				void render(const Geometry::AlignedBox<2> &, RealT z = 0.0) const;
				void render(const Geometry::AlignedBox<3> &) const;
			};
