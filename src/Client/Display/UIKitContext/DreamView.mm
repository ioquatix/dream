...
- (void)touchesBegan: (NSSet *)touches withEvent: (UIEvent *)event
{
	for (UITouch * touch in touches) {
		...
		MotionInput ipt(touchKey, Pressed, ft.position, ft.motion);
		UIKitContext::globalInputHandler()->process(ipt);
	}
}
...
- (void)touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
	for (UITouch * touch in touches) {
		...
		Key touchKey(DefaultTouchPad, ft.button);
		MotionInput ipt(touchKey, Dragged, ft.position, ft.motion);
		UIKitContext::globalInputHandler()->process(ipt);
	}
}
...
- (void)touchesEnded: (NSSet *)touches withEvent: (UIEvent *)event
{
	for (UITouch * touch in touches) {
		...
		Key touchKey(DefaultTouchPad, ft.button);
		MotionInput ipt(touchKey, Released, ft.position, ft.motion);
		UIKitContext::globalInputHandler()->process(ipt);
	}
}
